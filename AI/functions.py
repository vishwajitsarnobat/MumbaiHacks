...
async def generate_multilingual_content(
    gemini_model: genai.GenerativeModel, prompt: str, languages: List[str]
    ...
    try:
        ...
        try:
            ...
            content = {
                lang: (f"Title in {lang}", f"Description in {lang}")
        ...
        return {
            lang: (f"Title in {lang}", f"Description in {lang}") for lang in languages
        }


async def generate_image_prompts(
    gemini_model: genai.GenerativeModel, base_prompt: str, languages: List[str]
) -> Dict[str, str]:
    ...
    try:
        ...
        try:
            ...
            prompts = {
                lang: f"Advertisement for {base_prompt} in {lang} style"
                for lang in languages
            }
...
async def generate_image(
    ...
    try:
        async with session.post(
            stable_diffusion_url, headers=hf_headers, json={"inputs": prompt}
        ) as response:
...
async def generate_ad_content(
    location: Union[str, List[str]], age_group: str, languages: List[str], prompt: str
    ...
    try:
        ...
        hf_headers = {
            "Authorization": f"Bearer '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'"
        }
        ...
        text_content = await generate_multilingual_content(
            gemini_model, enhanced_prompt, languages
        )
            ...
            for lang in languages:
                try:
                    ...
                    image_bytes = await generate_image(
                        session, stable_diffusion_url, hf_headers, image_prompts[lang]
                    )
